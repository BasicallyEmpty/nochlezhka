...
const emailInput=document.querySelector('.popup-ticket-email');       // поле инпута с email
const ticketFormElement=document.querySelector('.popup-ticket-form'); //форма попапа покупки билета
...
ticketFormElement.addEventListener('submit', (evt) => {
  evt.preventDefault();
  const obj = {sum: sumTicket.value, sumPrice: priceTicket.textContent, email: emailInput.value};
  console.log(obj);
  closePopup(popupTicket);
